...
let fieldSceneryInterval = null;
let fieldSceneryTimeout = null;
const fieldSceneryImages = [
...
function removeFieldScenery(){
    gameArea.querySelectorAll(".field-item").forEach(item => item.remove());
}

function stopFieldScenery(){
    clearInterval(fieldSceneryInterval);
    clearTimeout(fieldSceneryTimeout);
    fieldSceneryInterval = null;
    fieldSceneryTimeout = null;
    removeFieldScenery();
}

function spawnFieldScenery(){
    if(!gameScreen.classList.contains("active")) return;
    if(gameArea.querySelectorAll(".field-item").length >= 8) return;

    const item = document.createElement("img");
    const width = gameArea.clientWidth;
    const grassWidth = width * .35;
    const size = Math.max(320, Math.min(430, grassWidth * (.5 + Math.random() * .16)));
    const side = Math.random() < .5 ? "left" : "right";
    const sideStart = side === "left" ? 0 : width * .65;
    const usableWidth = Math.max(10, grassWidth - size - 30);

    item.className = "field-item";
    item.src = fieldSceneryImages[Math.floor(Math.random() * fieldSceneryImages.length)];
    item.alt = "";
    item.setAttribute("aria-hidden", "true");
    item.style.width = `${size}px`;
    item.style.left = `${sideStart + 15 + Math.random() * usableWidth}px`;
    item.style.animationDuration = `${6.7 + Math.random() * .8}s`;
    item.style.setProperty("--field-tilt", `${-4 + Math.random() * 8}deg`);
    item.addEventListener("animationend", () => item.remove(), { once:true });
    gameArea.appendChild(item);
}

function startFieldScenery(){
    stopFieldScenery();
    spawnFieldScenery();
    fieldSceneryInterval = setInterval(spawnFieldScenery, 950);
}
...
backToMenuButton.addEventListener("click", () => {
    ...
    gameArea.removeAttribute("data-season");
...
()=>{
    ...
    // Change only the play-field appearance. Difficulty, questions and
    // the title-screen state remain independent of the seasonal theme.
    gameArea.dataset.season = selectedSeason;
...
function finishGame(){
    ...
    const finishTractor = document.getElementById("finish-tractor");
    ...
    const praiseMessages = [
        "Amazing work — you are a seasonal food superstar!",
        "Brilliant job — your food knowledge is growing!",
        "Fantastic exploring — the farm is cheering for you!"
    ];

    if(finishTitle) finishTitle.textContent = "YOU DID IT!";
    if(finishPraise) {
        finishPraise.textContent = praiseMessages[Math.floor(Math.random() * praiseMessages.length)];
    }
    finishSummary.textContent = `${seasonName} ${difficultyName} completed!`;
    finishScore.textContent = `${currentQuiz.length || questionsPerGame} questions completed`;
    finishTractor.src = `assets/tractors/${selectedSeason || "spring"}-drive.png`;
...
// Moving animals and plants were removed from the play field.
// They now appear only as a celebration group on the finish screen.

// Keep the copyright range current without requiring an annual HTML edit.
